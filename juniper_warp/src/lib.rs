...
where
    Query: coasys_juniper::GraphQLType<S, Context = CtxT> + Send + Sync + 'static,
    Query::TypeInfo: Send + Sync,
    Mutation: coasys_juniper::GraphQLType<S, Context = CtxT> + Send + Sync + 'static,
    Mutation::TypeInfo: Send + Sync,
    Subscription: coasys_juniper::GraphQLType<S, Context = CtxT> + Send + Sync + 'static,
    Subscription::TypeInfo: Send + Sync,
    CtxT: Send + Sync + 'static,
    CtxErr: Into<Rejection>,
    S: ScalarValue + Send + Sync + 'static,
...
async fn graphql_handler<Query, Mutation, Subscription, CtxT, S>(
    ...
    schema: Arc<coasys_juniper::RootNode<'static, Query, Mutation, Subscription, S>>,
    context: CtxT,
) -> reply::Response
where
    Query: coasys_juniper::GraphQLTypeAsync<S, Context = CtxT> + Send + 'static,
